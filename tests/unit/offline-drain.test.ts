...
/** The daily queue is addressed by date; the drain is told so explicitly. */
...
function sender(script: Record<string, SendOutcome>): Sender<PendingSave> & { calls: string[] } {
  const calls: string[] = []
  const fn = (async (entry: PendingSave) => {
    ...
    return outcome
  }) as Sender<PendingSave> & { calls: string[] }
  fn.calls = calls
  return fn
}
...
describe('drainPending', () => {
  it('sends every queued day, oldest first, and empties the queue', async () => {
    const store = memoryStore(byDate, [
      save('2026-09-16', 1),
      save('2026-09-17', 2),
      save('2026-09-18', 3),
    ])
    ...
    const report = await drainPending(store, send, byDate)
  ...
  it('keeps a day on a rejection it does not recognise', async () => {
    const store = memoryStore(byDate, [save('2026-09-18', 1)])
  ...
  it('leaves a day queued in another tab mid-drain alone', async () => {
    const store = memoryStore(byDate, [save('2026-09-16', 1)])
    const send: Sender<PendingSave> = async () => {
  ...
  it('does nothing, and says so, on an empty queue', async () => {
    const send = sender({})
    const empty = memoryStore<PendingSave>(byDate)
    expect(await drainPending(empty, send, byDate)).toEqual({ sent: 0, dropped: 0, kept: 0 })
...
describe('queuePending', () => {
  it('keeps the newest save of a day, not both', async () => {
    const store = memoryStore<PendingSave>(byDate)
