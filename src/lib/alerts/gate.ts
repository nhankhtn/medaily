 ...
 *
 * A route that throws on every render would otherwise send a message per
 * request. The gate is per-process and therefore imperfect on a platform that
 * runs many — it is there to turn a flood into a trickle, not to count.
 */
