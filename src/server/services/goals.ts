...
import { findGoals, findMilestonesFor } from '@/server/repositories/goals'
import type { BindableMetric } from '@/lib/metrics/bindable'
import { aggregateAnyMetric, bindableMetrics } from '@/server/services/metrics'
...
export const getGoalsView = cache(
  async (): Promise<{ today: ISODate; goals: GoalView[]; metrics: BindableMetric[] }> => {
    ...
    return { today, goals, metrics: await bindableMetrics(settings.userId) }
  },
)
