...
export async function parsePlan({
   ...
   * out in the prompt above and `toPlanItems` treats the answer as untrusted,
   * which it would have to do anyway.
   */
  const result = await generateJson<{ items?: ParsedPlanItem[] } | ParsedPlanItem[]>({
    systemInstruction: SYSTEM_PROMPT,
    input: [`Today: ${today}`, '', 'What they wrote:', text].join('\n'),
  })

  // Without a schema the model sometimes answers with the bare array.
  const parsed = Array.isArray(result) ? result : (result?.items ?? [])

  return toPlanItems({ parsed: Array.isArray(parsed) ? parsed : [], today })
}
