...
import { bindableMetrics } from '@/server/services/metrics'
...
export const getHabitsView = cache(
  async (): Promise<{ today: ISODate; habits: HabitView[]; metrics: BindableMetric[] }> => {
    ...
    return { today, habits, metrics: await bindableMetrics(settings.userId) }
  },
)
